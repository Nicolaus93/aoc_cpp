...
struct Line {
    ...
    Line(const Vec2D& p1, const Vec2D& p2) : p1(p1), p2(p2), is_horizontal(p1.y == p2.y) {}
    ...
    Vec2D get_intersection(const Line& other) const {
        if (this->is_horizontal && other.is_horizontal || !this->is_horizontal && !other.is_horizontal) {
            return Vec2D(0, 0);
        }
        ...
        if (this->is_horizontal) {
            ...
            if (min_x <= other_x && other_x <= max_x && y_min <= p1.y && p1.y <= y_max) {
    ...
    for (auto& segment1 : line1) {
        ...
        for (auto& segment2 : line2) {
            ...
            if (intersection != Vec2D()) {
                int steps = steps_1 + intersection.distance(segment1.p1) + steps_2 + intersection.distance(segment2.p1);
